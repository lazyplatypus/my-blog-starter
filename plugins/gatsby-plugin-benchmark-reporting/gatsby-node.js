...
class BenchMeta {
  ...
  getData() {
    ...
    const pageNum = parseInt(process.env.NUM_PAGES);
...
async function onPreInit(api) {
  lastApi = api
  init(`preInit`)
  reportInfo(`onpreinit triggered`)
...
async function onPreBootstrap(api) {
  lastApi = api
  init(`preBootstrap`)
  reportInfo(`prebootstrap triggered`)
...
async function onPreBuild(api) {
  lastApi = api
  init(`preBuild`)
  reportInfo(`onprebuild triggered`)
...
async function onPostBuild(api) {
  ...
  reportInfo(`onpostbuild triggered`)
